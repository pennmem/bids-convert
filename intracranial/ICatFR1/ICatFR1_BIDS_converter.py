...
class ICatFR1_BIDS_converter(intracranial_BIDS_converter):
    """Elemem / System-4 categorized free recall.

    Behaviorally the same paradigm as catFR1, so this mirrors
    ``catFR1_BIDS_converter`` closely. Two differences drive the deviations
    below, both verified across all 70 sessions in the r1 data index:
    ...
    * **Dead recognition columns.** ``recog_resp``, ``recog_rt``,
    ...
    # initialize
    def __init__(self, subject, experiment, session, system_version, unit_scale, area, brain_regions, overrides=None, root='/scratch/hherrema/BIDS/ICatFR1/'):
        super().__init__(subject, experiment, session, system_version, unit_scale, area, brain_regions, overrides, root)

    # ---------- Events ----------
    def set_wordpool(self):
        # Same cascade as catFR1 (minus its R1039M/R1094T special case — neither
        # subject ran ICatFR1). 69 of 70 sessions resolve to
        # wordpool_categorized_EN.txt. The exception is R1719S ses-2, which
        # presented Spanish items (NARIZ, CABEZA, PEGAMENTO) with English
        # category labels while its archived experiment_files/wordpool.txt is
        # the English pool — a source-data inconsistency. Its items match
        # neither shipped pool, so it falls through to 'n/a' until the lab
        # supplies the Spanish pool that was actually used.
    ...
    def events_to_BIDS(self):
        ...
        events.loc[(events.trial_type=='REC_WORD') | (events.trial_type=='REC_WORD_VV'),
                   'response_time'] = events['rectime'] / 1000.0                                         # use rectime (no PROB events in ICatFR1)
        ...
        # rejected), the constant `phase`, and the usual catFR1 exclusions
        ...
        events = events[self._append_uncorrected_cols(events, ['onset', 'duration', 'sample', 'trial_type', 'response_time', 'stim_file',
                        'item_name', 'category', 'serialpos', 'list', 'experiment', 'session', 'subject'])]    # re-order columns
