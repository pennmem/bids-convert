...
class catFR2_BIDS_converter(intracranial_BIDS_converter):
    wordpool_categorized_EN = np.loadtxt('/home1/hherrema/BIDS/bids-convert/intracranial/catFR2/wordpools/wordpool_categorized_EN.txt', dtype=str)
    wordpool_categorized_SP = np.loadtxt('/home1/hherrema/BIDS/bids-convert/intracranial/catFR2/wordpools/wordpool_categorized_SP.txt', dtype=str)
    ...
    def events_to_BIDS(self):
        ...
        events.loc[events.trial_type=='PRACTICE_WORD', 'list'] = range(1, len(events.query("type=='PRACTICE_WORD")) + 1)   # practice words given serial position = -999
    ...
    def apply_event_durations(self, events):
        ...
        for _, row in events.iterrows():
            # fixation events = 1600 ms               # 3600 ms between ORIENT and WORD, but going with same value as FR2, assumming longer ISI
