...
class FR1_Bids:
    ...
    wordpool_EN = np.loadtxt('wordpool_EN.txt', dtype=str)
    wordpool_ES = np.loadtxt('wordpool_ES.txt', dtype=str)
    wordpool_short_EN = np.loadtxt('wordpool_short_EN.txt', dtype=str)
    wordpool_long_EN = np.loadtxt('wordpool_long_EN.txt', dtype=str)
    wordpool_long_ES = np.loadtxt('wordpool_long_ES.txt', dtype=str)
    
    # initialize
    def __init__(self, subject, experiment, session, system_version, monopolar, mni, tal, root='~/programming_data/BIDS_convert/output_dir/'):
    ...
    def cml_reader(self):
        ...
        df_sess = df.query("subject==@self.session & experiment==@self.experiment & session==@self.sesion")
        self.reader = cml.CMLReader(subject=df_sess.iloc[0].subject, experiment = df_sess.iloc[0].experiment, session=df_sess.iloc[0].session, 
                                    localization=df_sess.iloc[0].localization, montage=df_sess.iloc[0].montage)
        
    # ---------- Events ----------
    def set_wordpool(self):
        evs = self.reader.load('events')
        word_evs = evs[evs['type']=='WORD']; word_evs = word_evs[word_evs['list']!=-1]    # remove practice list
        if np.all([1 if x in wordpool_EN else 0 for x in word_evs.item_name]):
                self.wordpool_file = 'wordpools/wordpool_EN.txt'
        elif np.all([1 if x in wordpool_short_EN else 0 for x in word_evs.item_name]):
            self.wordpool_file = 'wordpools/wordpool_short_EN.txt'
        elif np.all([1 if x in wordpool_long_EN else 0 for x in word_evs.item_name]):
            self.wordpool_file = 'wordpools/wordpool_long_EN.txt'
        elif np.all([1 if x in wordpool_ES else 0 for x in word_evs.item_name]):
            self.wordpool_file = 'wordpools/wordpool_ES.txt'
        elif np.all([1 if x in wordpool_long_ES else 0 for x in word_evs.item_name]):
    ...
    def contacts_to_electrodes(self, atlas):
        ...
        electrodes['size'] = -999                    # have to find this information
    ...
    def write_BIDS_electrodes(self, atlas):
        if atlas == 'tal':
            bids_path = mne_bids.BIDSPath(subject=self.subject, session=str(self.session), task=self.experiment, 
                                          suffix='electrodes', extension='.tsv', datatype='ieeg', 
                                          root=self.root)
            os.makedirs(bids_path.directory, exist_ok=True)                                 # make ieeg directory
            self.electrodes.to_csv(bids_path.fpath, sep='\t', index=False)
        elif atlas == 'mni':
            bids_path = mne_bids.BIDSPath(subject=self.subject, session=str(self.session), task=self.experiment, 
                                          suffix='electrodes', extension='.tsv', datatype='ieeg', 
                                          root=self.root)
    ...
    def eeg_metadata(self):              # set sample rate, system version
        ...
        self.recording_duration = eeg.data.shape[-1] / sfreq
    ...
    def eeg_sidecar(self, ref):
        ...
        elif self.system_verion >= 3.0 and self.system_version < 4.0:
    ...
    def write_BIDS_ieeg(self, ref):
        if ref == 'bipolar':
            bids_path = mne_bids.BIDSPath(subject=self.subject, session=str(self.session), task=self.experiment, 
                                          ...
                                          root=self.root)
            mne_bids.write_raw_bids(self.eeg_bi, bids_path=bids_path, events=None, overwrite=True)
            ...
            bids_path = mne_bids.BIDSPath(subject=self.subject, session=str(self.session), task=self.experiment, 
                                          ...
                                          root=self.root)
            mne_bids.write_raw_bids(self.eeg_mono, bids_path=bids_path, events=None, overwrite=True)
        ...
        bids_path = mne_bids.BIDSPath(subject=self.subject, session=str(self.session), task=self.experiment, 
                                      ...
                                      root='~/programming_data/BIDS_convert/output_dir/')
        self.events.to_csv(bids_path.fpath, sep='\t', index=False)
    ...
    def eeg_mono_to_BIDS(self):
        eeg = self.reader.load_eeg()
        eeg_mne = eeg.to_mne()
        eeg_mne.metadata = self.events         # add all events, defaults to only 1st event
        if self.system_version == 2.0:
            eeg_mne._data /= 4                 # convert from 250 nV to uV
        elif self.system_version >= 3.0 and self.system_version < 4.0:
            eeg_mne._data /= 10                # convert from 0.1 uV to uV
